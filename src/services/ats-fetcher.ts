 ...
 * Skips companies fetched in the last hour.
 */
  ...
  async (): Promise<{ total: number; companies: number }> => {
    try {
      ...
      const filteredRows = rows.filter((row) => {
        ...
        return atsSlugs.has(String(r.company_slug));
      });
      ...
      for (const row of filteredRows) {
        const r = row as Record<string, unknown>;
        const slug = String(r.company_slug);
        const name = String(r.company_name);

        // Check last fetch time for this company
      ...
      console.error("fetchAllWatchlistJobs error:", err);
...
/**
 * Refresh jobs feed — fetches all watchlist company jobs.
...
export const refreshJobsFeed = createServerFn({ method: "POST" }).handler(
  async (): Promise<{ total: number; companies: number }> => {
    return fetchAllWatchlistJobs({ data: undefined } as never);
  },
);
